...
export class ChatAgent implements BaseAgent {
  ...
  async initialize(): Promise<void> {
    if (this.initialized) {
      throw new Error('ChatAgent already initialized');
    }
  ...
  async execute(input: AgentInput, context: AgentContext): Promise<AgentOutput> {
    ...
    if (this.paused) {
      throw new Error('ChatAgent is paused');
    }
    if (this.disposed) {
      throw new Error('ChatAgent is disposed');
    }
  ...
  async resume(): Promise<void> {
    ...
    if (!this.paused) {
      throw new Error('ChatAgent is not paused');
    }
    this.paused = false;
  }
  ...
  async validate(input: AgentInput): Promise<boolean> {
    return input.type === 'chat_request' && input.data !== null;
  }
