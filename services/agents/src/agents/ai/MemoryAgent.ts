...
export class MemoryAgent implements BaseAgent {
  ...
  async initialize(): Promise<void> {
    if (this.initialized) {
      throw new Error('MemoryAgent already initialized');
    }
  ...
  async execute(input: AgentInput, context: AgentContext): Promise<AgentOutput> {
    ...
    if (this.paused) {
      throw new Error('MemoryAgent is paused');
    }
    if (this.disposed) {
      throw new Error('MemoryAgent is disposed');
    }
    ...
    return {
      ...
      data: {
        operation: 'Memory operation (mock)',
        input: input.data,
  ...
  async resume(): Promise<void> {
    ...
    if (!this.paused) {
      throw new Error('MemoryAgent is not paused');
    }
    this.paused = false;
  }
  ...
  async validate(input: AgentInput): Promise<boolean> {
    return input.type === 'memory_operation' && input.data !== null;
  }
