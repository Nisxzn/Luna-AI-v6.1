...
export class RAGAgent implements BaseAgent {
  ...
  async initialize(): Promise<void> {
    if (this.initialized) {
      throw new Error('RAGAgent already initialized');
    }
  ...
  async execute(input: AgentInput, context: AgentContext): Promise<AgentOutput> {
    ...
    if (this.paused) {
      throw new Error('RAGAgent is paused');
    }
    if (this.disposed) {
      throw new Error('RAGAgent is disposed');
    }
    ...
    return {
      type: 'rag_operation',
      data: {
        retrievedContext: 'Retrieved context (mock)',
        answer: 'RAG answer (mock)',
        input: input.data,
  ...
  async resume(): Promise<void> {
    ...
    if (!this.paused) {
      throw new Error('RAGAgent is not paused');
    }
    this.paused = false;
  }
  ...
  async validate(input: AgentInput): Promise<boolean> {
    return input.type === 'rag_operation' && input.data !== null;
  }
