...
export class DocumentationAgent implements BaseAgent {
  ...
  constructor(config: AgentConfig) {
    ...
    this.supportedModels = ['gpt-4', 'gpt-3.5-turbo', 'claude-3-opus'];
  ...
  async initialize(): Promise<void> {
    if (this.initialized) {
      throw new Error('DocumentationAgent already initialized');
    }
  ...
  async execute(input: AgentInput, context: AgentContext): Promise<AgentOutput> {
    if (!this.initialized) {
      throw new Error('DocumentationAgent not initialized');
    }
    if (this.paused) {
      throw new Error('DocumentationAgent is paused');
    }
    if (this.disposed) {
      throw new Error('DocumentationAgent is disposed');
    }
    ...
    return {
      type: 'documentation',
      data: {
        documentation: '# Generated documentation (mock)',
        input: input.data,
  ...
  async resume(): Promise<void> {
    ...
    if (!this.paused) {
      throw new Error('DocumentationAgent is not paused');
    }
    this.paused = false;
  }
  ...
  async validate(input: AgentInput): Promise<boolean> {
    return input.type === 'documentation' && input.data !== null;
  }
