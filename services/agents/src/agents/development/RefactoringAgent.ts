...
export class RefactoringAgent implements BaseAgent {
  readonly id: string;
  ...
  async initialize(): Promise<void> {
    if (this.initialized) {
      throw new Error('RefactoringAgent already initialized');
    }
  ...
  async execute(input: AgentInput, context: AgentContext): Promise<AgentOutput> {
    if (!this.initialized) {
      throw new Error('RefactoringAgent not initialized');
    }
    if (this.paused) {
      throw new Error('RefactoringAgent is paused');
    }
    if (this.disposed) {
      throw new Error('RefactoringAgent is disposed');
    }
  ...
  async resume(): Promise<void> {
    ...
    if (!this.paused) {
      throw new Error('RefactoringAgent is not paused');
    }
    this.paused = false;
  }
  ...
  async validate(input: AgentInput): Promise<boolean> {
    return input.type === 'refactoring' && input.data !== null;
  }
