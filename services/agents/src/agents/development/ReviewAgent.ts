...
export class ReviewAgent implements BaseAgent {
  ...
  async initialize(): Promise<void> {
    if (this.initialized) {
      throw new Error('ReviewAgent already initialized');
    }
  ...
  async execute(input: AgentInput, context: AgentContext): Promise<AgentOutput> {
    if (!this.initialized) {
      throw new Error('ReviewAgent not initialized');
    }
    if (this.paused) {
      throw new Error('ReviewAgent is paused');
    }
    if (this.disposed) {
      throw new Error('ReviewAgent is disposed');
    }
    ...
    return {
      type: 'review',
      data: {
        review: 'Code review (mock)',
        suggestions: 'Review suggestions (mock)',
        input: input.data,
  ...
  async resume(): Promise<void> {
    ...
    if (!this.paused) {
      throw new Error('ReviewAgent is not paused');
    }
    this.paused = false;
  }
  ...
  async validate(input: AgentInput): Promise<boolean> {
    return input.type === 'review' && input.data !== null;
  }
