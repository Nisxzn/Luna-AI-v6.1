...
export class TestingAgent implements BaseAgent {
  ...
  async initialize(): Promise<void> {
    if (this.initialized) {
      throw new Error('TestingAgent already initialized');
    }
  ...
  async execute(input: AgentInput, context: AgentContext): Promise<AgentOutput> {
    if (!this.initialized) {
      throw new Error('TestingAgent not initialized');
    }
    if (this.paused) {
      throw new Error('TestingAgent is paused');
    }
    if (this.disposed) {
      throw new Error('TestingAgent is disposed');
    }
    ...
    return {
      type: 'testing',
      data: {
        testResults: 'Test results (mock)',
        tests: '// Generated tests (mock)',
        input: input.data,
  ...
  async resume(): Promise<void> {
    ...
    if (!this.paused) {
      throw new Error('TestingAgent is not paused');
    }
    this.paused = false;
  }
  ...
  async validate(input: AgentInput): Promise<boolean> {
    return input.type === 'testing' && input.data !== null;
  }
