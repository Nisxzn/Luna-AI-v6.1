...
export class WorkspaceAgent implements BaseAgent {
  readonly id: string;
  ...
  async initialize(): Promise<void> {
    if (this.initialized) {
      throw new Error('WorkspaceAgent already initialized');
    }
  ...
  async execute(input: AgentInput, context: AgentContext): Promise<AgentOutput> {
    if (!this.initialized) {
      throw new Error('WorkspaceAgent not initialized');
    }
    if (this.paused) {
      throw new Error('WorkspaceAgent is paused');
    }
    if (this.disposed) {
      throw new Error('WorkspaceAgent is disposed');
    }
    ...
    return {
      ...
      data: {
        operation: 'Workspace operation (mock)',
        input: input.data,
  ...
  async resume(): Promise<void> {
    ...
    if (!this.paused) {
      throw new Error('WorkspaceAgent is not paused');
    }
    this.paused = false;
  }
  ...
  async validate(input: AgentInput): Promise<boolean> {
    return input.type === 'workspace_operation' && input.data !== null;
  }
